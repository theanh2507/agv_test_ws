...
class WaypointFollow(Node):
    def __init__(self):
        ...
        self.publisher_result = self.create_publisher(String, "/status_robot", 10)
    ...
    def receive_data_callback(self, msg:String):                    # khi an nut "move"
    ...
    def update_commands(self, msg: String):                         # khi an nut "finish"
        self.commands = msg.data
        print(f"Received command: {self.commands}")

        if self.commands == "go_to_next_point" and self.goal_result == True:
            if self.goal_index < len(self.data):
